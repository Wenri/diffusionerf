...
def get_rays(poses, intrinsics, H, W, N=-1, error_map=None):
    ...
    # Useful because it's the conversion factor to go from spherical to planar depths
    results['rays_d_cam_z'] = directions[..., -1]
    results['inds'] = inds
...
class Trainer(object):
    ...
    def train_step(self, data):
        ...
        outputs = self.model.render(rays_o, rays_d, staged=False, bg_color=bg_color, perturb=True, force_all_rays=False,
                                    **vars(self.opt))
            ...
            if self.local_rank == 0:
                ...
                if self.scheduler_update_every_step:
                    pbar.set_description(
                        f"loss={loss_val:.4f} ({total_loss / self.local_step:.4f}), lr={self.optimizer.param_groups[0]['lr']:.6f} global step={self.global_step}")
                else:
        ...
        with torch.no_grad():
            ...
            for data in loader:
                ...
                if self.local_rank == 0:
                    ...
                    path_weights_sum = os.path.join(self.workspace, 'validation',
                                                    f'{name}_{image_name}_weights_sum.png')
                    ...
                    cv2.imwrite(save_path_depth,
                                cv2.cvtColor((depth_cmapped * 255).astype(np.uint8), cv2.COLOR_RGB2BGR))
                    ...
                    if self.use_tensorboardX:
                        ...
                        self.writer.add_image('val/img', np.transpose(pred, axes=[2, 0, 1]),
